...
func (p *SQLiteProvider) initialize(path string) error {
	...
	_, err = db.Exec("CREATE TABLE IF NOT EXISTS U2FRegistrations (username VARCHAR(100) PRIMARY KEY, registration BLOB)")
	if err != nil {
		return err
	}
...
func (p *SQLiteProvider) SaveU2FRegistration(username string, keyHandle []byte) error {
	stmt, err := p.db.Prepare("INSERT OR REPLACE INTO U2FRegistrations (username, registration) VALUES (?, ?)")
	if err != nil {
		return err
	}
...
func (p *SQLiteProvider) LoadU2FRegistration(username string) ([]byte, error) {
	stmt, err := p.db.Prepare("SELECT registration FROM U2FRegistrations WHERE username=?")
	if err != nil {
		return nil, err
	}
	var keyHandle []byte
	err = stmt.QueryRow(username).Scan(&keyHandle)
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
